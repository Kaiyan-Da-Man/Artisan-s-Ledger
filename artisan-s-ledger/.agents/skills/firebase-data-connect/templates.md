...
```graphql
query ItemList($categoryId: UUID!) @auth(level: PUBLIC) @refresh(onMutationExecuted: { operation: "CreateItem", condition: "request.variables.categoryId == mutation.variables.categoryId" }) {
